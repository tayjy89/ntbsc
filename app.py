...
def main():
    ...
    with tabs[0]:
        ...
        edited = st.data_editor(consultants_df, num_rows="dynamic", use_container_width=True, key="edit_consultants")
        ...
        edited_d = st.data_editor(defaults_df, num_rows="dynamic", use_container_width=True, key="edit_room28_defaults")
    ...
    with tabs[1]:
        ...
        for idx, (fname, _) in enumerate(input_files):
            with cols[idx % 2]:
                st.markdown(f"**{fname}**")
                df = read_csv(fname)
                edited = st.data_editor(df, num_rows="dynamic", use_container_width=True, key=f"edit_{fname}")
    ...
    with tabs[3]:
        ...
        st.dataframe(ov, use_container_width=True, height=480)
