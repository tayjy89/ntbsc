...
def sg_public_holidays(year: int):
    fixed = {
        2024: ["2024-01-01","2024-02-10","2024-02-11","2024-03-29","2024-04-10","2024-05-01","2024-05-22","2024-06-17","2024-08-09","2024-10-31","2024-12-25"],
        2025: ["2025-01-01","2025-01-29","2025-01-30","2025-04-18","2025-05-01","2025-05-12","2025-06-06","2025-08-09","2025-10-20","2025-12-25"],
        2026: ["2026-01-01","2026-02-17","2026-02-18","2026-04-03","2026-05-01","2026-05-22","2026-06-26","2026-08-09","2026-11-09","2026-12-25"],
    }
...
def write_settings(s): json.dump(s, open(DATA_DIR / "settings.json","w"))
...
def _is_ph(d: date, year_ph_set: set): return d in year_ph_set
...
def build_roster(year, month, week_preceptor_mode, consultants_df, defaults_df, juniors_df, year_ph_set, duties_df, paeds_df, special_df):
    ...
    for i in roster.index:
        ...
        for col in ("Room 28","Preceptor"):
            ...
            if _has_woodlands(nm, d, duties_df):
                roster.loc[i, col] = ""; roster.loc[i,"notes"] = (roster.loc[i,"notes"]+f"; Woodlands({nm})").strip("; ")
            elif _has_leave(nm, d, session, duties_df):
                roster.loc[i, col] = ""; roster.loc[i,"notes"] = (roster.loc[i,"notes"]+f"; Leave({nm})").strip("; ")
        for col in ("Room 28","Preceptor"):
            nm = roster.loc[i, col]
            if nm and session=="AM" and _has_ward_am(nm, d, duties_df):
                roster.loc[i, col] = ""; roster.loc[i,"notes"] = (roster.loc[i,"notes"]+f"; WardRounds({nm})").strip("; ")
            if nm and session=="PM" and _has_blues_pm(nm, d, duties_df):
                roster.loc[i, col] = ""; roster.loc[i,"notes"] = (roster.loc[i,"notes"]+f"; Blues({nm})").strip("; ")
...
def calendar_markdown(roster_df: pd.DataFrame, year_ph_set: set):
    ...
    def cell_html(d):
        ...
        def fmt(slot):
            ...
            parts = []
            if slot["Room 28"]: parts.append(f"R28: {slot['Room 28']}")
            if slot["Preceptor"]: parts.append(f"P: {slot['Preceptor']}")
            if slot["Room 29"]: parts.append(f"R29: {slot['Room 29']}")
            return "; ".join(parts)
...
def main():
    ...
    with st.sidebar:
        ...
        mode = st.radio("Preceptor assignment", ["fixed","per_session"], index=0 if settings.get("week_preceptor_mode","fixed")=="fixed" else 1, key="mode")
    ...
    if roster_fp.exists():
        roster_df = pd.read_csv(roster_fp, dtype=str); roster_df["date"] = pd.to_datetime(roster_df["date"]).dt.date
    else:
        roster_df = build_roster(int(year), int(month), mode, consultants_df, defaults_df, juniors_df, year_ph, duties_df, paeds_df, special_df)
    ...
    if unlocked:
        with tabs[0]:
            ...
            if st.button("Save Consultants"): write_csv("consultants.csv", cedit); st.success("Saved consultants.csv")
            st.divider()
            st.subheader("Room 28 Defaults")
            ddef = st.data_editor(defaults_df, num_rows="dynamic", width="stretch", key="edit_room28_defaults")
            if st.button("Save Room28 Defaults"): write_csv("room28_defaults.csv", ddef); st.success("Saved room28_defaults.csv")
        ...
        with tabs[1]:
            ...
            ded = st.data_editor(
                ...
                num_rows="dynamic", width="stretch", key="duties_editor"
            )
            if st.button("Save Duties"): write_csv("duties.csv", ded); st.success("Saved duties.csv")
    ...
    with tabs[-2 if unlocked else 0]:
        ...
        ov_month = colov1.selectbox("Month (Overview/Calendar)", list(range(1,13)), index=int(month)-1, key="ov_month")
        ov_year = int(colov2.number_input("Year (Overview/Calendar)", min_value=2020, max_value=2100, value=int(year), step=1, key="ov_year"))
        ov_ph = sg_public_holidays(int(ov_year))
        ov_fp = ROSTER_DIR / f"{int(ov_year)}-{int(ov_month):02d}.csv"
        if ov_fp.exists():
            ov_roster = pd.read_csv(ov_fp, dtype=str); ov_roster["date"] = pd.to_datetime(ov_roster["date"]).dt.date
        else:
        ...
        md = calendar_markdown(ov_roster, ov_ph)
    ...
    with tabs[-1]:
        ...
        xmonth, xyear = int(ov_month), int(ov_year)
